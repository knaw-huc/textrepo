...
public interface MetadataDao {
  ...
  @Transaction
  @SqlBatch("insert into documents_metadata (document_id, key, value) values (:id, :key, :value)")
  @BatchChunkSize(1000)
  void bulkInsert(@Bind("id") UUID documentId, @BindBean Iterator<Map.Entry<String, String>> entries);

  @SqlQuery("select document_id, key, value from documents_metadata where document_id = ? and key = ?")
  ...
  @SqlQuery("select document_id, key, value from documents_metadata where document_id = ?")
  @RegisterConstructorMapper(MetadataEntry.class)
